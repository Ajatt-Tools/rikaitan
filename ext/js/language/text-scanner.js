...
export class TextScanner extends EventDispatcher {
    ...
    constructor({
        ...
        /** @type {boolean} */
        this._rikaitanIsChangingTextSelectionNow = false;
        /** @type {boolean} */
    ...
    setCurrentTextSource(textSource) {
        ...
        if (this._selectText && this._userHasNotSelectedAnythingManually && textSource !== null) {
            this._rikaitanIsChangingTextSelectionNow = true;
            textSource.select();
    ...
    _onSelectionChangeCheckUserSelection() {
        if (this._rikaitanIsChangingTextSelectionNow) { return; }
        this._userHasNotSelectedAnythingManually = this._computeUserHasNotSelectedAnythingManually();
    }
    ...
    async _findTermDictionaryEntries(textSource, optionsContext) {
        ...
        const {dictionaryEntries, originalTextLength} = await rikaitan.api.termsFind(searchText, details, optionsContext);
    ...
    async _findKanjiDictionaryEntries(textSource, optionsContext) {
        ...
        const dictionaryEntries = await rikaitan.api.kanjiFind(searchText, optionsContext);
        if (dictionaryEntries.length === 0) { return null; }
    ...
    async _hasJapanese(text) {
        try {
            return await rikaitan.api.textHasJapaneseCharacters(text);
        } catch (e) {
            return false;
        }
    }
