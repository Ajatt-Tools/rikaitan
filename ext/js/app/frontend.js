...
export class Frontend {
    ...
    async prepare() {
        ...
        try {
            const {zoomFactor} = await rikaitan.api.getZoom();
        ...
        rikaitan.on('optionsUpdated', this.updateOptions.bind(this));
        rikaitan.on('zoomChanged', this._onZoomChanged.bind(this));
        rikaitan.on('closePopups', this._onClosePopups.bind(this));
    ...
    async _updateOptionsInternal() {
        ...
        const options = await rikaitan.api.optionsGet(optionsContext);
    ...
    async _getIframeProxyPopup() {
        ...
        const {popupId} = await rikaitan.crossFrame.invoke(targetFrameId, 'Frontend.getPopupInfo', {});
        if (popupId === null) {
            return null;
        }
    ...
    _signalFrontendReady(targetFrameId) {
        ...
        if (targetFrameId === null) {
            rikaitan.api.broadcastTab('frontendReady', params);
        } else {
            rikaitan.api.sendMessageToFrame(targetFrameId, 'frontendReady', params);
        }
    }
    ...
    async _waitForFrontendReady(frameId, timeout) {
        return new Promise((resolve, reject) => {
            ...
            rikaitan.api.broadcastTab('Frontend.requestReadyBroadcast', {frameId: this._frameId});
        });
    }
    ...
    async _getSearchContext() {
        ...
        if (this._useProxyPopup && this._parentFrameId !== null) {
            try {
                ({url, documentTitle} = await rikaitan.crossFrame.invoke(this._parentFrameId, 'Frontend.getPageInfo', {}));
            } catch (e) {
                // NOP
            }
        }
    ...
    async _prepareGoogleDocsWrapper() {
        ...
        await rikaitan.api.loadExtensionScripts([
