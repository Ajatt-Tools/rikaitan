...
export class RikaitanApi {
    ...
    async _onMessage(message) {
        ...
        if (this._port !== null) {
            ...
            try {
                ...
                let statusCode = 200;
                switch (action) {
                    case 'rikaitanVersion': {
                        const {version} = chrome.runtime.getManifest();
                        result = {version: version};
                        break;
                    }
                    case 'termEntries': {
                        /** @type {import('rikaitan-api.js').termEntriesInput} */
                    ...
                    case 'kanjiEntries': {
                        /** @type {import('rikaitan-api.js').kanjiEntriesInput} */
                        // @ts-expect-error - Allow this to error
                    ...
                    case 'ankiFields': {
                        /** @type {import('rikaitan-api.js').ankiFieldsInput} */
                        // @ts-expect-error - Allow this to error
                    ...
                    case 'ankiCardFormats': {
                        /** @type {import('rikaitan-api.js').ankiCardFormatsInput} */
     ...
     * @returns {Promise<import('rikaitan-api.js').apiDictionaryMediaDetails[]>}
     */
    async _fetchDictionaryMedia(dictionaryEntries) {
        /** @type {import('rikaitan-api.js').apiDictionaryMediaDetails[]} */
        const media = [];
        let mediaCount = 0;
        for (const dictionaryEntry of dictionaryEntries) {
            ...
            for (const mediaFileData of mediaFilesData) {
                ...
                const ankiFilename = await mediaFileNameHashOrTimestamp('rikaitan_dictionary_media', mediaFileData.content, getFileExtensionFromImageMediaType(mediaFileData.mediaType) ?? '', mediaCount, timestamp);
                media.push({
     ...
     * @param {import('dictionary.js').DictionaryEntry[]} dictionaryEntries
     * @param {import('settings').ProfileOptions} options
     * @returns {Promise<import('rikaitan-api.js').apiAudioMediaDetails[]>}
     */
    async _fetchAudio(dictionaryEntries, options) {
        ...
        for (const dictionaryEntry of dictionaryEntries) {
            ...
            try {
                ...
                const ankiFilename = await mediaFileNameHashOrTimestamp('rikaitan_audio', audioData.data, extension, null, timestamp);
                audioDatas.push({
                    term: headword.term,
                ...
                log.log('Rikaitan API failed to download audio ' + toError(e).message);
            }
        }
        return audioDatas;
    }
    ...
    async _updateRemoteVersion(url) {
        if (!url) {
            throw new Error('Missing Rikaitan API URL');
        }
        try {
            const response = await fetch(url + '/serverVersion', {
                method: 'POST',
            });
            /** @type {import('rikaitan-api.js').remoteVersionResponse} */
    ...
    _onDisconnect() {
        ...
        const error = new Error(e ? e.message : 'Rikaitan Api disconnected');
        log.error(error);
        this._clearPort();
    }
     ...
     */
    async _setupPort() {
        const port = chrome.runtime.connectNative('rikaitan_api');
