...
export class BackupController {
    ...
    async _getSettingsExportData(date) {
        ...
        const environment = await rikaitan.api.getEnvironmentInfo();
        const fieldTemplatesDefault = await rikaitan.api.getDefaultAnkiFieldTemplates();
    ...
    async _onSettingsExportClick() {
        ...
        const fileName = `rikaitan-settings-${this._getSettingsExportDateString(date, '-', '-', '-', 6)}.json`;
    ...
    async _onSettingsExportDatabaseClick() {
        ...
        try {
            ...
            const fileName = `rikaitan-dictionaries-${this._getSettingsExportDateString(date, '-', '-', '-', 6)}.json`;
    ...
    _databaseImportProgressCallback({totalRows, completedRows, done}) {
        ...
        if (done) {
            ...
            messageContainer.textContent = 'Done importing. You will need to re-enable the dictionaries and refresh afterward. If you run into issues, please restart the browser. If it continues to fail, reinstall Rikaitan and import dictionaries one-by-one.';
        }
    }
    ...
    async _onSettingsImportDatabaseChange(e) {
        ...
        try {
            ...
            this._databaseExportImportErrorMessage('Encountered errors when importing. Please restart the browser and try again. If it continues to fail, reinstall Rikaitan and import dictionaries one-by-one.');
        } finally {
            pageExitPrevention.end();
            this._settingsExportDatabaseToken = null;
        }
