...
async function showAnkiConnectInfo() {
    ...
    try {
        ankiConnectVersion = await rikaitan.api.getAnkiConnectVersion();
    } catch (e) {
        // NOP
    }
...
async function showDictionaryInfo() {
    let dictionaryInfos;
    try {
        dictionaryInfos = await rikaitan.api.getDictionaryInfo();
    } catch (e) {
        return;
    }
...
async function main() {
    try {
        ...
        const {name, version, manifest_version} = manifest;
        const {browser, platform: {os}} = await rikaitan.api.getEnvironmentInfo();
        ...
        /** @type {HTMLElement} */
        const manifestVersionElement = querySelectorNotNull(document, '#manifest_version');
