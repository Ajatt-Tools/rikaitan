...
export async function loadStyle(id, type, value, useWebExtensionApi = false, parentNode = null) {
    if (useWebExtensionApi && rikaitan.isExtensionUrl(window.location.href)) {
    ...
    if (type === 'file-content') {
        value = await rikaitan.api.getStylesheetContent(value);
        type = 'code';
        useWebExtensionApi = false;
    }
    ...
    if (useWebExtensionApi) {
        ...
        await rikaitan.api.injectStylesheet(type, value);
        return null;
    }
